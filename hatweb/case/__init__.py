...
from base import build_driver, build_logger, BoxDriver, PathHelper, Logger, YamlHelper, DictHelper, parse_dict
...
class BaseTest(object):
    ...
    def init_driver(self, url: str, driver_type="c"):
        """
        初始化测试用例的驱动对象，传递给 self.__driver
        :param driver_type: 驱动类型
        :param url: 驱动的网址：需要指明 协议
        :return: None
        """
        self.info("[%s] - 系统初始化驱动，请求数据：%r" % (__name__, dict(url=url, driver_type=driver_type)))
        self.__driver = build_driver(url=url, driver_type=driver_type)
    ...
    def close_driver(self):
        ...
        """
        self.info("[%s] - 系统关闭驱动！")
        if self.driver is not None and isinstance(self.driver, BoxDriver):
            self.driver.quit()
    ...
    def snapshot(self, title=None):
        ...
        if self.driver is not None and isinstance(self.driver, BoxDriver):
            image = self.driver.save_window_snapshot_by_png()
            allure.attach(image, name=title, attachment_type=allure.attachment_type.PNG)
